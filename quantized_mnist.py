...
# connect x and quantized version of x together? why ??? kernel size?
def make_pair(x):
...
class QuantConv2d(nn.Module):
    ...
    def forward(self, x):
        ...
        #nn.conv2d will hide w from coder so use F.conv2d to access both x and w
        x = F.conv2d(x, w, self.bias, self.stride, self.padding,
                     self.dilation, self.groups)

        return x

###try to write QuantLinear
